...
non-positive-+-has-joins .has-joins.joinl {x} {y} =
  from-op≤ $ ≤-strengthen $ min-≤l x y
non-positive-+-has-joins .has-joins.joinr {x} {y} =
  from-op≤ $ ≤-strengthen $ min-≤r x y
non-positive-+-has-joins .has-joins.universal {x} {y} {z} z≤x z≤y =
  from-op≤ $ ≤-strengthen $ min-is-glb (to-≤ (to-op≤ z≤x)) (to-≤ (to-op≤ z≤y))
...
NonPositive+⊆Int+ .is-displacement-subalgebra.into .homo .is-displacement-algebra-homomorphism.strictly-mono = negate-anti-mono
