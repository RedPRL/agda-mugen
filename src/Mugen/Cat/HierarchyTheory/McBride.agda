...
ℳ {o = o} 𝒟 = ht where
  ...
  M .F₁ {L} {N} f .strict-mono {l1 , d1} {l2 , d2} =
    ∥-∥-rec (×-is-hlevel 1 squash $ Π-is-hlevel 1 λ _ → Poset.has-is-set (M .F₀ L) _ _) λ where
      (biased l1=l2 d1≤d2) → inc (biased (ap (f .hom) l1=l2) d1≤d2) , λ p → ap₂ _,_ l1=l2 (ap snd p)
      (centred l1≤l2 d1≤ε ε≤d2) → inc (centred (mono f l1≤l2) d1≤ε ε≤d2) , λ p →
        ap₂ _,_ (inj-on-related f l1≤l2 (ap fst p)) (ap snd p)
  M .F-id = trivial!
  M .F-∘ f g = trivial!

  unit : Id => M
  unit .η L .hom l = l , ε
  unit .η L .strict-mono l1≤l2 = inc (centred l1≤l2 ≤-refl ≤-refl) , ap fst
  unit .is-natural L L' f = trivial!

  mult : M F∘ M => M
  mult .η L .hom ((l , x) , y) = l , (x ⊗ y)
  mult .η L .strict-mono {(a1 , d1) , e1} {(a2 , d2) , e2} =
    ∥-∥-rec (×-is-hlevel 1 squash $ Π-is-hlevel 1 λ _ → Poset.has-is-set (M .F₀ (M .F₀ L)) _ _) lemma where
      ...
      lemma (biased ad1=ad2 e1≤e2) =
        ...
        λ p i → ad1=ad2 i , injr-on-related e1≤e2 (ap snd p ∙ ap (_⊗ e2) (sym $ ap snd ad1=ad2)) i
      lemma (centred ad1≤ad2 e1≤ε ε≤e2) = ∥-∥-map lemma₂ ad1≤ad2 , lemma₃ where
        ...
        lemma₃ p i = (a1=a2 i , d1=d2 i) , e1=e2 i where
          ...
          e1=e2 : e1 ≡ e2
          e1=e2 = injr-on-related (≤-trans e1≤ε ε≤e2) $ ap snd p ∙ ap (_⊗ e2) (sym d1=d2)
